...
import { getCompletedExperiments, incrementCompletedExperiments } from './completionBadge.js'
...
export default function ActivityScreen({ experiment, onBack }) {
    ...
    <main className="relative min-h-screen w-full bg-gradient-to-br from-rainbow-purple/35 via-curio-background to-rainbow-green/35 p-6 text-curio-text md:p-10">
      ...
      <section className="relative mx-auto max-w-5xl">
        ...
        <header className="flex flex-col items-center pt-16 text-center">
          <p className="text-sm font-extrabold uppercase tracking-[0.2em] text-rainbow-pink">Try it at home</p>
        ...
        <div className="mt-6 space-y-6">
          <section className="rounded-[2rem] border-2 border-rainbow-blue bg-rainbow-blue/20 p-6 shadow-lg md:p-8">
            <h2 className="mb-5 text-lg font-black uppercase tracking-[0.18em] text-sky-800 md:text-xl">What you&apos;ll need</h2>
            <ul className="space-y-4 text-xl font-semibold md:text-2xl">
              {experiment.materials.map((material) => (
                <li key={material} className="flex items-start gap-4">
                  <span aria-hidden="true" className="mt-1 flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border-2 border-rainbow-blue bg-curio-background text-sm text-sky-800">✓</span>
                  <span>{material}</span>
                </li>
              ))}
            </ul>
          ...
          <section>
            <h2 className="mb-5 text-center text-lg font-black uppercase tracking-[0.18em] text-rainbow-purple md:text-xl">Let&apos;s make it</h2>
            <ol className="space-y-6">
              {experiment.steps.map((step, index) => (
                <li key={`${index}-${step}`} className={`flex gap-5 rounded-[2rem] border-2 p-6 shadow-lg md:p-8 ${stepStyles[index % stepStyles.length]}`}>
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-curio-background text-xl font-black shadow-sm">{index + 1}</span>
                  <p className="pt-1 text-xl font-semibold leading-8 text-curio-text md:text-2xl">{step}</p>
                </li>
              ))}
            </ol>
          </section>
          ...
          <aside className="rounded-[2rem] border-2 border-rainbow-orange bg-rainbow-orange/25 p-6 shadow-lg md:p-8">
            <div className="flex gap-4">
              ...
              <div>
                <h2 className="text-lg font-black uppercase tracking-[0.18em] text-orange-800 md:text-xl">Ask a grown-up to help!</h2>
                <p className="mt-4 text-xl font-semibold leading-8 md:text-2xl">{experiment.safetyNote}</p>
              </div>
            </div>
          </aside>
          ...
          <section className="rounded-[2rem] border-2 border-rainbow-purple bg-rainbow-purple/20 p-6 shadow-lg md:p-8">
            ...
            >
              <span className="text-lg font-black uppercase tracking-[0.18em] text-violet-800 md:text-xl">Curious why?</span>
              <span aria-hidden="true" className="text-3xl font-black text-violet-800">{whyVisible ? '−' : '+'}</span>
            </button>
            {whyVisible && <p className="animate-punchline-reveal mt-5 text-xl font-semibold leading-8 md:text-2xl">{experiment.whyItWorks}</p>}
          </section>

          <div className="relative pb-6 text-center">
            {isDone && <div aria-hidden="true" className="animate-celebration pointer-events-none absolute inset-x-0 -top-10 text-4xl">🎉 ✨ 🎊</div>}
            <button
              type="button"
              onClick={markDone}
              disabled={isDone}
