        ...
        <div className="mt-6 grid gap-6 md:grid-cols-2">
          <div className="min-w-0 space-y-6">
            <RevealCard delay={100} className="border-rainbow-blue bg-rainbow-blue/20">
              <p className="mb-5 text-lg font-black uppercase tracking-[0.18em] text-sky-800 md:text-xl">Fun fact</p>
              <p className="text-xl font-semibold leading-8 md:text-2xl">{fact}</p>
            </RevealCard>

            <RevealCard delay={200} className="border-rainbow-yellow bg-rainbow-yellow/25">
              <p className="mb-5 text-lg font-black uppercase tracking-[0.18em] text-amber-800 md:text-xl">Joke time</p>
              <button
                type="button"
              ...
              >
                {punchlineVisible ? (
                  <span className="animate-punchline-reveal block text-xl leading-8 md:text-2xl">{joke}</span>
                ) : (
                  <span className="block text-xl md:text-2xl">Tap to reveal punchline! Ready for a joke about {theme}?</span>
                )}
              </button>
            </RevealCard>
          </div>

          <RevealCard delay={300} className="flex h-full min-h-80 min-w-0 flex-col border-rainbow-green bg-rainbow-green/20">
            <p className="mb-5 text-lg font-black uppercase tracking-[0.18em] text-green-800 md:text-xl">Try it at home</p>
            <h2 className="text-4xl font-black">{experiment.title}</h2>
            <p className="mt-5 text-xl leading-8 md:text-2xl">A hands-on {theme.toLowerCase()} activity with everyday materials.</p>
            <button
              type="button"
